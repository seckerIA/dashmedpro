...
export interface PendingAction {
  type: 'navigate' | 'openModal' | 'toast' | 'highlight' | 'closeModal';
  payload: unknown;
}
...
function emitAction(action: PendingAction) {
  ...
  if (actionCallback) {
    ...
    console.warn('[clientTools] ALERTA: Callback de ação é NULL. Ação perdida:', action);
  }
}
  ...
  try {
    const tools = {
      ...
      getAgenda: async (params: GetAgendaParams): Promise<ToolResult> => {
        console.log('[clientTools] 🔧 getAgenda chamado com params:', params);
        try {
          let startDate: Date;
          let endDate: Date;
      ...
      /**
       * Fecha a interface da Cortana
       */
      close_cortana: async (): Promise<ToolResult> => {
        emitAction({
          type: 'closeModal',
          payload: {},
        });
    ...
    console.error('[clientTools] Erro ao criar tools:', error);
    return {};
  }
}
