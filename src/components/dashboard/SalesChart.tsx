...
export function SalesChart({ data, title, className }: SalesChartProps) {
  return (
    <div className={`bg-card rounded-2xl p-3 sm:p-4 lg:p-6 border border-border ${className}`}>
      {title && (
        <h3 className="text-sm sm:text-base lg:text-lg font-semibold mb-3 sm:mb-4 lg:mb-6 text-foreground">{title}</h3>
      )}
            ...
            <Tooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-card border border-border rounded-lg p-2 sm:p-3 shadow-xl">
...
export function WeeklySalesChart({ data, title, className }: WeeklySalesChartProps) {
  ...
  return (
    <div className={`bg-card rounded-2xl p-3 sm:p-4 lg:p-6 border border-border ${className}`}>
      ...
      <div className="flex items-end justify-between gap-1 sm:gap-2 lg:gap-3 h-48 sm:h-56 lg:h-64">
        {data.map((item, index) => {
          ...
          return (
            <div key={index} className="flex-1 flex flex-col items-center gap-1 sm:gap-2">
              <div className="relative w-full flex items-end justify-center h-32 sm:h-40 lg:h-48 group">
                {/* Barra */}
                <div
                ...
                >
                  ...
                  <div className="absolute -top-6 sm:-top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="bg-card border border-border rounded-lg px-1.5 sm:px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs font-semibold shadow-xl whitespace-nowrap">
