...
// Mock types since full Cortana system is not ported yet
type CortanaStatus = 'idle' | 'listening' | 'processing' | 'speaking' | 'connecting' | 'error';

interface CortanaState {
...
export function CortanaButton({
    ...
    const isConfigured = true; // Always true for visual stub

    const handleClick = () => {
        // Simulate interaction
        if (state.status === 'idle') {
            setState(prev => ({ ...prev, status: 'listening' }));
            setTimeout(() => setState(prev => ({ ...prev, status: 'processing' })), 3000);
            setTimeout(() => setState(prev => ({ ...prev, status: 'speaking' })), 4500);
            setTimeout(() => setState(prev => ({ ...prev, status: 'idle' })), 6500);
        } else {
    ...
    const getTooltipText = (): string => {
        ...
        switch (state.status) {
            ...
            default:
                return 'Clique para ativar a Cortana (Visual Demo)';
        }
    };
