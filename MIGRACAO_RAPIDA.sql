...
-- ========================================
-- PARTE 1: Limpar estruturas existentes (se houver conflitos)
-- ========================================

-- Drop policies existentes que podem conflitar
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.tablename || '_select" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "' || r.tablename || '_insert" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "' || r.tablename || '_update" ON public.' || r.tablename;
        EXECUTE 'DROP POLICY IF EXISTS "' || r.tablename || '_delete" ON public.' || r.tablename;
    END LOOP;
END $$;
